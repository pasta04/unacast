...
なお、chatウィンドウと翻訳ウィンドウの背景色はOBS向けの半透過色とは別に、既定でダークモード相当の `#1e1e1e` が適用されています ([style-window.css](../public/css/style-window.css))。上記のユーザーカスタムCSSで `body { background-color: ... }` を書けば、この既定色も上書きできます。
