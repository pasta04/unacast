    ...
    <SectionPanel title="レス・コメント着信音設定">
      <Box sx={{ maxWidth: 600 }}>
        ...
        <TextField
          ...
          onChange={(e) => setConfig('sePath', e.target.value)}
        />

        <Box sx={{ mt: 1 }}>
          <FormControlLabel
            control={<Checkbox checked={config.playSe} onChange={(e) => setConfig('playSe', e.target.checked)} size="small" />}
            label="着信音再生を有効にする"
          />
          <FormControlLabel
        ...
        <Box sx={{ mt: 2 }}>
          ...
          {audioOutputs.map((d) => (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={config.audioOutputDevices.includes(d.deviceId)}
                  onChange={(e) => toggleDevice(d.deviceId, e.target.checked)}
                  size="small"
                />
              }
