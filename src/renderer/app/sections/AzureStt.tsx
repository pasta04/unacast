...
export const AzureStt: React.FC = () => {
  ...
  const update = (patch: Partial<AppConfig['azureStt']>) =>
    setConfig('azureStt', { ...config.azureStt, ...patch });
  ...
  return (
    <SectionPanel title="Azure音声認識">
      ...
      <FormControlLabel
        control={
          <Checkbox size="small" checked={config.azureStt.enable} onChange={(e) => update({ enable: e.target.checked })} disabled={isServerRunning} />
        }
        label="有効にする"
      />
      ...
      <Box sx={{ maxWidth: 600 }}>
        ...
        <TextField
          ...
          size="small"
          fullWidth
          value={config.azureStt.inputDevice}
          onChange={(e) => update({ inputDevice: e.target.value })}
          disabled={isServerRunning}
        >
          {audioInputs.length === 0 && (
            <MenuItem value="default">既定のデバイス</MenuItem>
          )}
          {audioInputs.map((d) => (
